...
def maximize_pressure_2(graph, flow_rates, starts=("AA", "AA"), dsts=(26, 26), visited=None):
    ...
    for neigh1 in graph[s1]:
        ...
        for neigh2 in graph[s2]:
            ...
            if all(no_move):
                continue
            sub_pressure = maximize_pressure_2(graph, flow_rates, tuple(new_starts), tuple(new_dsts), new_visited)
    ...
    memoization[memoization_key] = best_pressure
    # Returns the best path, followed by total pressure
...
def main():
    ...
    # Test data
    print(find_min_dst("AA", "CC", graph))
